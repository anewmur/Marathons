...
def create_temporal_stability_plot() -> None:
    ...
    # ============================================================
    # 3. Выбираем трассу с максимумом калибровочных бегунов
    # ============================================================

    print("\nАнализируем трассы для выбора оптимальной...")

    # Сначала определяем калибровочных бегунов ДЛЯ КАЖДОГО ГОДА
    # (те, кто бегал ≥2 трассы в один год)

    calibration_runners_by_year = {}

    for year in sorted(df['year'].unique()):
        df_year = df[(df['year'] == year) & (df['gender'] == 'M')]
        runner_races = df_year.groupby('runner_id')['race_id'].nunique()
        calibration_runners_by_year[year] = set(
            runner_races[runner_races >= 2].index
        )

    # Теперь для каждой трассы считаем сколько калибровочных бегунов
    race_calibration_stats = []

    for race_id in df['race_id'].unique():
        df_race = df[(df['race_id'] == race_id) & (df['gender'] == 'M')]

        # Годы этой трассы
        years_in_race = sorted(df_race['year'].unique())

        if len(years_in_race) < 3:
            continue  # Минимум 3 года

        # Считаем калибровочных бегунов по годам
        total_calibration = 0
        years_with_calibration = 0

        for year in years_in_race:
            if year not in calibration_runners_by_year:
                continue

            # Кто из калибровочных бегал эту трассу в этом году?
            df_race_year = df_race[df_race['year'] == year]
            runners_this_year = set(df_race_year['runner_id'])

            calibration_this_year = runners_this_year & calibration_runners_by_year[year]

            if len(calibration_this_year) >= 20:  # Минимум 20 калибровочных
                total_calibration += len(calibration_this_year)
                years_with_calibration += 1

        if years_with_calibration >= 3:  # Минимум 3 года с калибровочными
            race_calibration_stats.append({
                'race_id': race_id,
                'total_n': len(df_race),
                'n_years': len(years_in_race),
                'years_with_calibration': years_with_calibration,
                'total_calibration': total_calibration,
                'avg_calibration_per_year': total_calibration / years_with_calibration,
            })

    if len(race_calibration_stats) == 0:
        raise RuntimeError("Нет трасс с достаточным числом калибровочных бегунов")

    df_race_stats = pd.DataFrame(race_calibration_stats)
    df_race_stats = df_race_stats.sort_values('total_calibration', ascending=False)

    # Показываем ТОП-5 кандидатов
    print("\nКандидаты (≥3 года с ≥20 калибровочными бегунами/год):")
    for i, row in df_race_stats.head(5).iterrows():
        print(f"  {row['race_id']:30s}: "
              f"{row['total_calibration']:4.0f} калибровочных ({row['years_with_calibration']} лет), "
              f"{row['total_n']:5.0f} всего участников")

    # Выбираем трассу с максимумом калибровочных
    target_race = df_race_stats.iloc[0]['race_id']

    print(f"\n✓ ВЫБРАНА: {target_race} (максимум калибровочных бегунов)")
    ...
    print(
        f"✓ Сезонных linking (≥2 трассы в один год): {n_season_linking} ({100 * n_season_linking / len(df_race):.1f}%)")
    ...
    for year in years:
        ...
        if len(df_year_experienced) >= 50:
            ...
            if len(delta_experienced) >= 4:
                ...
                print(
                    f"  {year} (опыт): {len(df_year_experienced)} наблюдений, {len(delta_experienced)} возрастных бинов")
        ...
        if len(df_year_linking) >= 20:  # СНИЗИЛИ порог с 30 до 20
            delta_linking = estimate_delta_by_year(df_year_linking, spline, age_min, age_max)

            print(f", {len(delta_linking)} возрастов", end='')

            if len(delta_linking) >= 3:  # СНИЗИЛИ порог с 4 до 3
                deltas_season_linking[year] = delta_linking
                print(f" ✓")
            else:
                print(f" ✗ (мало возрастов с данными)")
        else:
            print(f" ✗ (недостаточно бегунов)")
    ...
    if len(deltas_experienced) >= 2:
        ...
        print(
            f"   В натуральной шкале: {100 * (np.exp(mean_spread_exp) - 1):.1f}% - {100 * (np.exp(max_spread_exp) - 1):.1f}%")
    ...
    if len(deltas_season_linking) >= 2:
        ...
        print(
            f"   В натуральной шкале: {100 * (np.exp(mean_spread_link) - 1):.1f}% - {100 * (np.exp(max_spread_link) - 1):.1f}%")
